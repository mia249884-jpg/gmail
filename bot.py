...
DEFAULT_CONFIG = {
    "bot_token": "8695687684:AAGzqWpSPLjJEEWRpx-7BT9MLIetmhQoj-c",
    "admin_ids": [576819773],
    ...
    "group_bot": {
        "enabled": True,
        "token": "8695687684:AAGzqWpSPLjJEEWRpx-7BT9MLIetmhQoj-c",
...
# ==========================
# GOOGLE SHEETS
# Sheet columns: A: chat_id  B: username  C: full_name  D: joined_at
# ==========================
SCOPES = ["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"]
_sheet_obj = None

# In-memory cache — alias/tag data local এ, sheet এ শুধু user info
_cache     = {}  # chat_id(str) -> {tag, email, tag_history, username, full_name, ...}
_tag_index = {}  # tag.lower() -> chat_id(str)

# Local JSON files — alias data শুধু local এ থাকবে
ALIAS_FILE   = "alias_data.json"   # chat_id -> {tag, email, tag_history}
...
def get_sheet():
    global _sheet_obj
    if _sheet_obj: return _sheet_obj
    try:
        cfg    = load_config()
        creds  = Credentials.from_service_account_file("credentials.json", scopes=SCOPES)
        client = gspread.authorize(creds)
        _sheet_obj = client.open(cfg["sheet_title"]).sheet1
        return _sheet_obj
    except Exception as e:
        print(f"[Sheet connect] {e}")
        return None
...
def init_sheet_headers():
    ...
    try:
        ...
        if not row1 or row1[0] != "chat_id":
            sheet.update("A1:D1", [["chat_id","username","full_name","joined_at"]])
            print("✅ Sheet headers set")
    except Exception as e:
        print(f"[Sheet headers] {e}")

def load_cache():
    """Sheet (user list) + local alias file থেকে memory তে load করে"""
    global _cache, _tag_index
    sheet = get_sheet()
    if not sheet:
        print("[Cache] Sheet connect failed")
        return
    try:
        # Sheet থেকে user list load
        ...
        for i, row in enumerate(rows[1:], start=2):
            ...
            _cache[cid] = {
                ...
                "joined_at": row[3] if len(row) > 3 else "",
                "tag":       "",
                "email":     "",
                "tag_history": [],
                "row":       i
            }
        # Local alias file থেকে alias data merge করো
        alias_data = _load_alias_file()
        _tag_index = {}
        for cid, adata in alias_data.items():
            if cid not in _cache:
                _cache[cid] = {
                    "chat_id": cid, "username": "", "full_name": "",
                    "joined_at": "", "row": None
                }
            _cache[cid]["tag"]         = adata.get("tag", "")
            _cache[cid]["email"]       = adata.get("email", "")
            _cache[cid]["tag_history"] = adata.get("tag_history", [])
            # tag index বানাও
            for t in adata.get("tag_history", []):
                _tag_index[t.lower()] = cid
            if adata.get("tag"):
                _tag_index[adata["tag"].lower()] = cid
        print(f"✅ Cache loaded: {len(_cache)} users")
    except Exception as e:
        print(f"[Cache load] {e}")

def save_user_sheet(cid, data):
    """শুধু A-D (user info) sheet এ save করে"""
    sheet = get_sheet()
    if not sheet: return
    try:
        values = [
        ...
        if row_idx:
            sheet.update(f"A{row_idx}:D{row_idx}", [values])
        else:
            sheet.append_row(values)
            rows = sheet.get_all_values()
            for i, r in enumerate(rows, start=1):
                if r and str(r[0]) == str(cid):
                    _cache[str(cid)]["row"] = i
                    break
    except Exception as e:
        print(f"[Sheet save] {e}")

def save_alias_local(cid):
    """Alias data local file এ save করে"""
...
def register_user(chat_id, username, full_name):
    """User প্রথমবার /start দিলে sheet এ A-D save করে"""
    cid = str(chat_id)
    if cid in _cache:
    ...
    _cache[cid] = data
    # Background: sheet এ শুধু A-D
    threading.Thread(target=lambda: save_user_sheet(cid, data), daemon=True).start()
...
def send_mail_to_group(subject, from_, to_, body, otp=None, tag=None):
    ...
    try:
        ...
        if not r.get("ok"):
            print(f"[Group bot Error] {r.get('description')}")
            return
...
def admin_panel(chat_id, _=None):
    cfg = load_config()
                ...
                if "callback_query" in update:
                    ...
                    if data == "refresh_alias":
                        ...
                        if data == "adm_add_gmail":
                            ...
                            save_config(cfg); admin_panel(chat_id)
...
if __name__ == "__main__":
    ...
    for gmail in cfg["gmail_list"]:
        ...
        if pw and "YOUR_APP_PASSWORD" not in pw:
            ...
            print(f"⚠️ Password নেই: {gmail}")

    print("🤖 Bot চালু...")
    try:
        update_telegram_menu_button()
    except Exception as e:
        print(f"[Menu Button Init Error] {e}")
    run_bot()
