...
def load_cache():
    ...
    sheet = get_sheet()
    if not sheet:
        print("[Cache] Sheet connect failed, fallback to local alias data only")
    else:
        try:
...
def run_bot():
    ...
    while True:
        try:
            ...
            if updates:
                print(f"📥 Received {len(updates)} updates: {updates}")
